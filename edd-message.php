    ...
    function add_user_comment_edd() {
        ...
        if ($user['id'] == get_current_user_id() || $vendor == get_current_user_id()) {
            ...
            if ($thread_status != 'close') {
                $html .= '<h3>' . __('Masseges', WBCOM_EDD_DASH_MSG_TEXT_DOMIAN) . '</h3>
                ...
                $html .= '</div>';
//                $html .= '<input type="file" multiple name="attach[]" id="edd_message_attachment" style="display:none;" /><label for="edd_message_attachment" class="edd_message_attachment">' . __('<img src="' . get_site_url() . '/wp-content/plugins/edd-service-extended/includes/images/attachment1.png" alt="Attach Files" width="20" height="20" align="center">', WBCOM_EDD_DASH_MSG_TEXT_DOMIAN) . '</label><input type="submit" name="add_comment" value="Send" class="submit-msg" /><div id="edd_files_names"></div><input type="hidden" value="' . $vendor . '" name="edd_vendor"><input type="hidden" value="' . $edd_receipt_args['id'] . '" name="purchase_id">' . wp_nonce_field('edd_message_action', 'edd_message_nonce_field', false, false) . '</form>';
                $html .= '<div class="send-btn"><input type="submit" name="add_comment" value="Send" class="submit-msg" /><div id="edd_files_names"></div><input type="hidden" value="' . $vendor . '" name="edd_vendor"><input type="hidden" value="' . $edd_receipt_args['id'] . '" name="purchase_id">' . wp_nonce_field('edd_message_action', 'edd_message_nonce_field', false, false) . '</div></form>';
            }
            $html .= '</div>';
        }
        return $html;
...
if (!function_exists('get_conversation_by_pay_id')) {
    ...
    function get_conversation_by_pay_id($id) {
        ...
        $table_name = $wpdb->prefix . 'edd_dashboard_message';
        $msg = $wpdb->get_results("SELECT * FROM $table_name WHERE purchase_id = $id ORDER BY msg_time ASC");
...
if (!function_exists('wbcom_edd_download_history')) {
    ...
    add_shortcode('wbcom_download_history', 'wbcom_edd_download_history');
}
if (!function_exists('wbcom_plugin_activate')) {

    function wbcom_plugin_activate() {
        ...
        $page_id = wp_insert_post($pg_args);
        $pg_args1 = array(
            'post_type' => 'page',
    ...
    register_activation_hook(__FILE__, 'wbcom_plugin_activate');
}
...
//	edd_get_template_part( 'shortcode', 'receipt' );
    ...
    add_shortcode('wbcom_edd_receipt', 'wbcom_edd_receipt_shortcode');
}

if (!function_exists('get_log_ids')) {
...
if (!function_exists('get_payment_ids')) {
